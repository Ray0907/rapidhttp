...
Benchmark RapidHTTP vs requests using public APIs
...
def check_api_ready(url, max_retries=3):
    """Check if API is accessible"""
    for i in range(max_retries):
        try:
            r = test_requests.get(url, timeout=5)
...
def benchmark_library(lib, lib_name, url, num_requests=10):
...
def benchmark_json_parser(lib, lib_name, url, iterations=10):
...
def benchmark_json_post(lib, lib_name, url, payload, num_requests=10):
    """Test JSON POST and parsing performance"""
    print(f"\nTesting {lib_name} JSON POST...")
    
    # Warmup
    for _ in range(3):
        try:
            lib.post(url, json=payload, timeout=10)
        except Exception as e:
            print(f"  Warmup error: {e}")
            return None
    ...
    for i in range(num_requests):
        try:
            start = time.perf_counter()
            r = lib.post(url, json=payload, timeout=10)
...
def run_benchmark():
    ...
    # Test 1: GET requests to JSONPlaceholder API
    print(f"\n{'=' * 70}")
    print("  Test 1: GET Requests (JSONPlaceholder API)")
    print(f"{'=' * 70}")
    
    test_url = "https://jsonplaceholder.typicode.com/posts/1"
    print(f"Checking API {test_url} ...")
    
    if not check_api_ready(test_url):
        print("\n⚠️  API not accessible, skipping GET test")
    else:
        print("✓ API ready")
        
        num_requests = 10
        print(f"Configuration: {num_requests} requests")
    ...
    # Test 2: JSON POST requests to JSONPlaceholder API
    print(f"\n{'=' * 70}")
    print("  Test 2: JSON POST Requests (JSONPlaceholder API)")
    print(f"{'=' * 70}")
    
    json_url = "https://jsonplaceholder.typicode.com/posts"
    payload = {
        'title': 'Performance Test',
        'body': 'Testing RapidHTTP vs requests performance',
        'userId': 1,
        'metadata': {
            'timestamp': time.time(),
    ...
    num_json_requests = 10
    
    print(f"Configuration: {num_json_requests} requests, payload size: {payload_size} bytes")
    
    json_results = []
    for lib_name, lib in libs.items():
        result = benchmark_json_post(lib, lib_name, json_url, payload, num_json_requests)
        if result:
    ...
    # Test 3: JSON Parser Performance with Real APIs
    print(f"\n{'=' * 70}")
    print("  Test 3: JSON Parser Performance (Real API Responses)")
    print(f"{'=' * 70}")

    # Test with different endpoint sizes
    test_endpoints = {
        'Small': ('https://jsonplaceholder.typicode.com/posts/1', 5),
        'Medium': ('https://jsonplaceholder.typicode.com/posts', 10),
        'Large': ('https://jsonplaceholder.typicode.com/comments', 20),
    }

    for size_name, (endpoint, iterations) in test_endpoints.items():
        print(f"\n{size_name} JSON from {endpoint} ({iterations} iterations):")

        parser_results = []
        for lib_name, lib in libs.items():
            result = benchmark_json_parser(lib, lib_name, endpoint, iterations)
            if result:
    ...
    print(f"\n{'=' * 70}")
    print("Note: Network latency may affect results. Run multiple times for consistency.")
    print(f"{'=' * 70}\n")
